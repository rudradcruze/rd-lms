...
- Keep coverage for auth flows, role management, user management, and error envelope regressions.  
  <!-- Derived from: tests/auth.test.js, tests/roles.test.js, tests/users.test.js -->

## 11) Config/Formatting Rules
...
## 12) Known Constraints / Guardrails
...
## 13) Mandatory Post-Feature Workflow
...
4. Run migrations/seed as needed, then run `npm test` (and `npm run test:coverage` when relevant).
5. Update `MEMORY.md` to reflect new architecture/routes/models/rules-impact before finalizing.
